...
forma_pagto AS (
    SELECT 
        pedidos_vendas_identificador as pedido_id, 
        MAX(LOWER(COALESCE(forma_pagamento_desc, ''))) as forma_pagto 
    FROM `iron-rex-461220-g4.database_aroom_health.pedidos_vendas_parcelas` 
    GROUP BY 1
),
marketing_diario AS (
    SELECT 
        DATE(data) as data_mkt, 
        SUM(spend) as custo_mkt_dia 
